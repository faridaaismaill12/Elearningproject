...
const QuestionsTable: React.FC = () => {
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:4000/instructor/quizzes/module/${moduleId}`, {
  ...
  const handleDelete = async (questionId: string) => {
    try {
      await axios.delete(`http://localhost:4000/instructor/quizzes/delete/question/${questionId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.patch(`http://localhost:4000/instructor/quizzes/update/question/${currentQuestion._id}`, updatedQuestion, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchQuestions();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:4000/instructor/quizzes/add`, newQuestion, {
        headers: { Authorization: `Bearer ${token}` },
      });
      resetForm();
      fetchQuestions();
