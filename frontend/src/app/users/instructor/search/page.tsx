...
export default function SearchStudentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.get("http://localhost:5010/users/search-students", {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-6 lg:px-20">
      <div className="bg-white p-10 rounded-lg shadow-lg max-w-4xl w-full">
        <h2 className="text-3xl font-semibold mb-6 text-center">Search Students</h2>
        {error && (
          ...
          <>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {["name", "email", "studentLevel", "enrolledCourseId", "bio"].map((field) => (
                  <div key={field}>
                    <label
                      htmlFor={field}
                ...
                <table className="w-full border-collapse border border-gray-300 text-left">
                  <thead>
                    <tr>
                      ...
                      <th className="border border-gray-300 p-3">Enrolled Courses</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.map((result: any, index: number) => (
                      <tr key={index}>
                        ...
                        <td className="border border-gray-300 p-3">{result.studentLevel}</td>
                        <td className="border border-gray-300 p-3">{result.bio}</td>
                        <td className="border border-gray-300 p-3">
                          {result.enrolledCourses?.join(", ") || "None"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
